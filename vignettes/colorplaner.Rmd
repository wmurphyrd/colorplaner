...
In _The Visual Display of Quantitative Information_, Edward Tufte praises map 
visualizations as inherently highly
...
When Y is held constant, the remaining color space is a U-V plane which 
appears as if delineated into four quadrants of color: green, orange, magenta, 
and blue. 
...
Mapping data variables to these U and V components transforms this color plane into a 
...
whether the values each mapped point are i
the values of the mapped 
variables can be readily interpreted in terms of 

for general values of the
mapped cariables to be readily ascertained
faciliates rapid identification of 
The most extreme values will appear as the saturated shades of the 
color for the quadrant, and values closer to the mid-point of the range will
be mapped to colors resuling from 
and values closer to the midpoints of variables will appear less saturated, leading to grey at the . 

This creates the 
appearance of a scale divided into four quadrants, 
facilitate
with
with
red and pink colors marking the border of the orange and magenta quadrants, 
purples and blues delineating 
intersection regions with shades of olive, pink, purple, and teal. 
Colors are also more saturated near the extremes and desaturated (gray) at the center.

, and using these colors for the `color` or `fill` aesthetics of 
plotted objects allows the viewer to easily approximate the values of
the two mapped variables and to discern relative values by comparing colors. 
...
Points *b*, *d*, and *n* appear in shades of fuchsia, blue, and purple, 
respectively. This indicates that all three have sepal lengths in the upper 
half of the data range and that they can be ranked from longest to shortest sepal width 
as *b*, *n*, and *d*.

Points *a*, *f*, and *o* have colors in the olive region at the intersection
of the green and red quadrants, indicating shorter sepal lengths and increasing
sepal width can be ranked by decreasing amounts of green: *o*, *f*, *a*.
...
in the lower left quadrant of the plot and blue, purple, and fuchsia in the upper right
indicates that sepal length increases with petal length and petal width. 
Conversely, the presence of olive and blue alongside green and fuchsia reveals
that sepal width is not simply correlated with the other variables. 
Analysis with Pearson's correlation coefficient confirms these observations. 
...
sepal length is small, yet the lack of bright blues or fuchsias also indicates
...
intersections of color regions and the location of the grey center. Finally,
